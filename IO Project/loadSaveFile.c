...
static void LoadWeapon(FILE *playerFile, struct character *character) {
    fscanf(playerFile, "%i", &character->weapon);
}

static void LoadCharacterAttacks(FILE *playerFile, struct character *character) {
    int i = 0;

    fscanf(playerFile, "%i", &character->attackQuantity);
    character->attacks = malloc(sizeof(int) * character->attackQuantity);

    while (i < character->attackQuantity) {
        fscanf(playerFile, "%i", &character->attacks[i]);
...
static void LoadCharacter(struct character *character, const char *characterSave, float x, float z) {
    ...
    *object = (struct Object2D){
    ...
    LoadWeapon(playerFile, character);

    LoadCharacterAttacks(playerFile, character);
